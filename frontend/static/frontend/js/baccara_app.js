...
function updateLogicButtonsUI(selectedValue) {
    ...
    if (isMember) {
        // 비동기로 서버에 로직 변경을 통보합니다.
        fetch(PROCESS_GAME_RESULT_API_URL, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                action: 'update_logic_selection', // 새로운 액션 타입
                selectedLogic: selectedValue,
            })
        })
            .then(response => {
                if (!response.ok) {
                    console.error('서버에 로직 변경 통보 실패:', response.status);
                    // showAlertModal 등으로 사용자에게 알릴 수도 있지만,
                    // UI 변경에 직접적인 영향을 주지 않으므로 콘솔 로그만 남깁니다.
                } else {
                    console.log(`서버에 로직 변경 통보 완료: ${selectedValue}`);
                }
            })
            .catch(error => {
                console.error('서버에 로직 변경 통보 중 네트워크 오류:', error);
            });
    }
}
...
async function loadStateFromBackend(serverState = null) {
    ...
    try {
        ...
        if (!stateToLoad) {
            // GET 요청 시 현재 로컬 스토리지의 selectedLogic을 쿼리 파라미터로 전송합니다.
            const currentSettings = loadSavedSettings();
            const selectedLogicQuery = currentSettings.selectedLogic ? `?selectedLogic=${currentSettings.selectedLogic}` : '';

            const response = await fetch(`${PROCESS_GAME_RESULT_API_URL}${selectedLogicQuery}`, { // <-- 쿼리 파라미터 추가
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': CSRF_TOKEN
            ...
            if (response.status === 204) {
                ...
                // 백엔드로부터 받은 selectedLogic으로 로컬 스토리지 동기화 (필요시)
                if (result.selectedLogic && result.selectedLogic !== currentSettings.selectedLogic) {
                    currentSettings.selectedLogic = result.selectedLogic;
                    localStorage.setItem(GAMEENV_KEY, JSON.stringify(currentSettings));
                    console.log(`서버로부터 받은 로직으로 로컬 스토리지 업데이트: ${result.selectedLogic}`);
                    updateLogicButtonsUI(result.selectedLogic); // UI도 업데이트
                }
            }
        }
        ...
        if (stateToLoad && stateToLoad.full_logic_state) {
            ...
            const currentSettings = loadSavedSettings(); // 최신 설정 다시 로드 (서버 동기화 후)
