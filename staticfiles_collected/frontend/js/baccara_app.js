...
const HEARTBEAT_API_URL = DJANGO_CONTEXT.heartbeatApiUrl; // <-- API URL 로드
const CSRF_TOKEN = DJANGO_CONTEXT.csrfToken; // <-- CSRF 토큰 로드
...
let heartbeatInterval = null; // 초기값은 null
...
async function updateAnalyticsChart() {
    if (!inlineChartInstance) {
        ...
        if (ctx && typeof Chart !== 'undefined') {
            ...
            console.error("Chart.js context or instance could not be initialized.");
            return;
        }
    }
    ...
    try {
        ...
        const dummyChartData = {
            ...
            'virtual_betting': { win_rate: Math.floor(Math.random() * 30) + 50, count: Math.floor(Math.random() * 50) + 80 } // <--- 가상배팅 더미 데이터 추가
        };
        ...
        for (const key in dummyChartData) {
            // 라벨을 '본매' 등으로 바꾸는 대신 '로직1', '로직2' 등으로 유지
            ...
            else if (key === 'virtual_betting') logicDisplayName = '가상배팅'; // <--- 가상배팅 라벨
        ...
        // inlineChartInstance.data.datasets[0].backgroundColor (색상 배열도 5개로 늘려야 함)
        inlineChartInstance.data.datasets[0].backgroundColor = [
            'rgba(59, 130, 246, 0.7)', // Logic 1: 파란색
            'rgba(239, 68, 68, 0.7)', // Logic 2: 빨간색
            'rgba(34, 197, 94, 0.7)', // Logic 3: 녹색
            'rgba(168, 85, 247, 0.7)', // Logic 4: 보라색
            'rgba(245, 158, 11, 0.7)'  // Virtual Betting: 주황색 (amber-500)
        ];
        ...
        // datalabels의 위치를 막대 "안쪽 끝"으로 조정 (anchor: 'end', align: 'end')
        inlineChartInstance.options.plugins.datalabels.anchor = 'end'; // 막대의 끝 부분
        inlineChartInstance.options.plugins.datalabels.align = 'start'; // 막대 안쪽으로 정렬
...
function showAlertModal(options) {
    ...
    // 'alert-modal'이 수신할 커스텀 이벤트를 생성하고 발송합니다.
    window.dispatchEvent(new CustomEvent('show-custom-alert', {
        detail: {
            title: options.title || '알림',
            message: options.message || '',
            icon: options.icon || 'info',
            onConfirm: options.onConfirm || (() => { /* no-op */ })
        }
    }));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --------------------------------------------------------------------------------------------------
    // Heartbeat 및 Broadcast Message 로직 구현
    // --------------------------------------------------------------------------------------------------
    heartbeatInterval = setInterval(async () => {
        ...
        try {
            const heartbeatResponse = await fetch(HEARTBEAT_API_URL, {
                ...
                body: JSON.stringify({ /* no data needed for session refresh */ })
            });
            ...
            if (!heartbeatResponse.ok) {
                if (heartbeatResponse.status === 401 || heartbeatResponse.status === 403) {
                    ...
                    // 오류 응답에도 JSON 본문이 없을 수 있으므로, 먼저 본문 존재 여부 확인
        ...
        // 2. Broadcast Message (공지 메시지) 확인 요청 (수정)
        try {
            ...
            // 204 No Content는 본문이 없으므로 json()을 호출하면 안 됩니다.
            if (broadcastResponse.status === 204) { // <-- 204 상태 코드를 가장 먼저 확인합니다!
                // console.log("Broadcast: 활성 공지 메시지가 없습니다."); // 정상적인 경우이므로 콘솔 메시지는 줄입니다.
                return; // JSON 파싱 없이 함수 종료
            }

            if (broadcastResponse.ok) { // 200 OK 등 다른 성공적인 응답 (본문이 있을 것으로 예상)
            ...
            } else { // 200 OK 또는 204 No Content가 아닌 다른 오류 응답
                // 오류 응답에도 JSON 본문이 없을 수 있으므로, 먼저 본문 존재 여부 확인
            ...
            // 네트워크 오류 시 사용자에게 알림 (heartbeat와 동일하게 처리)
        }
